...
public class ConnectionController {
    ...
    public ResponseEntity<Void> connect(@RequestParam int userId, @RequestParam String countryName) throws Exception{
        ...
        //3. Else, the user should be subscribed under a serviceProvider having option to connect to the given country.
            ...
            //Else, establish the connection where the maskedIp is "updatedCountryCode.
            // serviceProviderId.userId" and return the updated user. If multiple service providers allow you to connect to
            // the country, use the service provider having smallest id.
    ...
    public ResponseEntity<Void> communicate(@RequestParam int senderId, @RequestParam int receiverId) throws Exception{
        ...
        //The sender is initially not connected to any vpn. If the sender's original country does not match receiver's
           // current country, we need to connect the sender to a suitable vpn. If there are multiple options,
           // connect using the service provider having smallest id
        //If the sender's original country matches receiver's current country, we do not need to do anything as they
           // can communicate. Return the sender as it is.
